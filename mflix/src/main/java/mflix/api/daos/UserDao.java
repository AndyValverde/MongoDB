...
public class UserDao extends AbstractMFlixDao {
    ...
    public boolean createUserSession(String userId, String jwt) {
    	...
    	if(Optional.ofNullable(sessionsCollection.find(eq("user_id",userId)).first()).isPresent()) {
          ...
          sessionsCollection.insertOne(sesion);  
        }**/
    	Document sesion = new Document();
    	sesion.append("user_id", userId);
    	sesion.append("jwt", jwt);
    	sessionsCollection.insertOne(sesion);
    	return true;
    ...
    public Session getUserSession(String userId) {
    	...
    	Document s= sessionsCollection.find(eq("user_id", userId)).first();
    	if(s != null) {
    		sesion.setJwt(s.getString("jwt"));
    		sesion.setUserId(userId);
    ...
    public boolean updateUserPreferences(String email, Map<String, ?> userPreferences) {
    	...
    	usersCollection.updateOne(eq("email",email), set("preferences",userPreferences));
    		...
    		throw new IncorrectDaoOperation("Referencias no pueden ser null");
    	}
        //TODO > Ticket: Handling Errors - make this method more robust by
