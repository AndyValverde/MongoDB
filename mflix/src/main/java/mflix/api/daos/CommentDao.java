...
public class CommentDao extends AbstractMFlixDao {
    ...
    public Comment addComment(Comment comment) {
    	...
    	if(comment.getId()!=null) {
    		...
    		throw new IncorrectDaoOperation("Id no reconocido");
    	}
    ...
    public boolean updateComment(String commentId, String text, String email) {
    	...
    	if(commentId!= null && email != null) { 
    		resultado = commentCollection.updateOne(and( eq("_id",new ObjectId(commentId)), eq("email", email)),
    				new Document("$set",new Document("text",text).append("date",new Date())));
    	}else {
    		throw new IncorrectDaoOperation("Id no reconocido");
    	}
        //return resultado.wasAcknowledged() && resultado != null;
    	return resultado.getMatchedCount() > 0 && resultado.getModifiedCount() > 0;
    }
    ...
    public boolean deleteComment(String commentId, String email) {
    	...
    	DeleteResult borrar;
    	if(commentId != null) {
    		 borrar = commentCollection.deleteMany(and(eq("_id",new ObjectId(commentId)),eq("email",email)));
    	}else {
    		throw new IllegalArgumentException("id no valido");
    	}
        return borrar.getDeletedCount() > 0;
    }
